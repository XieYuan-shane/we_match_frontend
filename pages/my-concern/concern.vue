<template>
	<view class="concern-view" v-if="userArticleData.length > 0" v-for="(item,index) in userArticleData" :key="index">
...
<script setup>
	...
	async function requestRes(){
		try{
			...
			userArticleData.value = userFauthor.data
	...
	async function giveAlike(index,isLiked,_id){
		if(isLiked){//取消点赞
			...
			userArticleData.value[index].isLiked = false
			userArticleData.value[index].likes--
		}else{//点赞
			await requestApi('/user-like',{article_id:_id})
			userArticleData.value[index].isLiked = true
			userArticleData.value[index].likes++
		}
	}
	// 收藏和取消收藏
	async function operCollection(index,isCollect,_id){
		if(isCollect){//取消收藏
			...
			userArticleData.value[index].isCollect = false
			userArticleData.value[index].collections--
		}else{//收藏
			await requestApi('/collect-article',{article_id:_id})
			userArticleData.value[index].isCollect = true
			userArticleData.value[index].collections++
		}
	}
	...
	onReachBottom(async()=>{
		...
		userArticleData.value = [...userArticleData.value,...userFauthor.data]
	})
	// 登录
	function goLogin(){
