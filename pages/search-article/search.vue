<template>
	<view class="search-view">
		<view class="search-input">
			...
			<input type="text" placeholder="搜索活动/作者" 
			@input="updateInput"
			@focus="focusInput"
			@confirm="onSearch"
			v-model="keywords"
	...
	<!-- 搜索历史和热门搜索内容 v-show="historyShow"-->
	...
	<view class="Hot-area global-display global-a-items fadeIn" 
		v-for="(item,index) in hotArea" :key="index"
		@click="goSearch(item._id)">
			<image :src="item.image" mode="aspectFill"></image>
			<view>
				<text>{{item._id}}</text>
				<text>{{item.count}}个活动</text>
			</view>
		</view>
	</view>
	<!-- 显示实时搜索列表 -->
...
// 搜索的作者
import searchAuthorView from '@/pages/search-article/components/search-author.vue'
// 瀑布流活动
...
const updateInput = debounceapi(async()=>{
	...
	// 请求搜索列表
	const res = await requestApi('/search-activity',{keywords:keywords.value})
...
// 进入页面获取本地缓存的搜索历史和热门活动
onMounted(async()=>{
	const res = wx.getStorageSync('searchKeywords')
	historyData.value = res || []
	const hotData = await requestApi('/hot-area')
	hotArea.value = hotData.data
})
// 清空搜索历史
