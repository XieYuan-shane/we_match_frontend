...
<grid-view type="masonry" cross-axis-count="2"> 
	<view class="item-card fadeIn" v-for="(item,index) in userArticles" :key="index" @click="juMp(item._id,item.fileType)">
...
const props = defineProps({
	userArticles: {type:Array,required:true}
})
const exist = ref(false)
watch(props,(newVal)=>{
	// console.log(newVal);
	exist.value = newVal.userArticles.length > 0 ? false : true
})
/* 
