<template>
	...
	<view class="Mainstream-view global-display global-a-items" v-if="newestArticle.length > 0">
	  <!-- 左边 -->
		<view class="Mainstream-left" 
		@click="juMP(newestArticle[int].article_id,newestArticle[int].fileType)">
	    <image class="Mainstream-left-image fadeIn" 
		:src="newestArticle[int].imageUrl"
		mode="aspectFill"
		></image>
	    <view class="Mainstream-bottom">
	      <p class="m-title text-show">{{newestArticle[int].title}}</p>
	      <view class="global-display global-a-items">
		  <image :src="newestArticle[int].avatarUrl" mode="aspectFill"></image>
	      <p class="m-name">{{newestArticle[int].nickname}}</p>
			<p class="m-name text-show dis-flex">我在&nbsp;{{newestArticle[int].address}}</p>
	      </view>
	    </view>
	  </view>
		<!-- 右边 -->
	  <view class="Mainstream-right global-display global-f-direction">
		<image v-for="(item,index) in newestArticle" :key="index" :src="item.imageUrl"
		mode="aspectFill"
...
<script setup>
	import {ref} from 'vue'
	defineProps({newestArticle:Array})
	// 切换推荐
	const int = ref(0)
