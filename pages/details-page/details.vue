...
<share-element key="yourkey" transition-on-gesture> 
	...
	<!-- 发起组队 -->
	<view class="initial-team-formation">
     <button v-if = "needGroup" hover-class="none" class="form-companion" @click="initPartner">发起组队</button>
	 <button v-if = "grouped" hover-class="none" class="go-companion" @click="goIntoCompanion">进入组队</button>
	</view>
	<!-- 相关推荐 -->
	<view v-show="userArticles.length > 0">
		<text class="rec-the-same">相关推荐</text>
		<card :userArticles="userArticles"/>
		<view class="View-more" v-if="userArticles.length >= 6" @click="loadMore">{{moreRecommend}}</view>
	</view>
	<view style="height: 200px;"></view>
	<!-- 底部：点赞，评价数，收藏 -->
	<view class="bottom-oper global-display global-a-items global-j-content">
		<view class="bottom-oper-feel">
			<input hold-keyboard placeholder="评论" disabled @click="geComment"/>
		</view>
...
function initPartner(){
    const url = '/pages/companion/Init-partner?articleId='+articleId.value;
    wx.navigateTo({url: url});
}
// 跳转详情
async function goIntoCompanion(){
	// 查询是否登录
	await requestApi('/check-login');
	wx.navigateTo({url:'/pages/companion/partner-details?articleId=' + articleId.value})
}
async function detailsArticle(){
	// 文章详情
	...
	const {likes,comments,collections,isLike,isCollecTions} = article_data.data
	...
	// 相关推荐文章
	const user_articles = await requestApi('/rec-the-same',{article_id:articleId.value,page:1})
	userArticles.value = user_articles.data
	//作者id
	authorId = article_data.data.author_data[0].uid
	const res = await requestApi('/getArticleIsGroup',{article_id:articleId.value,author_id:authorId})
	if(res.data === '001'){
...
async function operatingState(index){
	if(index === 0){//操作点赞
		...
		if(bottomOper.value[2].status){//取消收藏
			...
			await requestApi('/collect-article',{article_id:articleId.value})
...
async function loadMore(){
	rec_page.value++
	const user_articles = await requestApi('/rec-the-same',{article_id:articleId.value,page:rec_page.value})
	userArticles.value = [...userArticles.value,...user_articles.data]
	if(user_articles.data.length <= 0){
