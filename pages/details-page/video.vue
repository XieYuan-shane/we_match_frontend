...
<view class="search-video">
	...
	<view class="global-display global-a-items">
		...
		<view class="search-input" @click="juMp">
			...
			<input type="text" placeholder="搜索文章" placeholder-class="input-color" disabled>
		</view>
	</view>
</view>
<!-- :id="`myVideo${index}`" -->
...
<script setup>
	...
	async function detailsArticles(){
		apiPage.value = 1
		// 视频详情：初次进入第一个
		...
		// 获取相关推荐视频
		const user_articles_vedio = await requestApi('/rec-the-video',{article_id:articleId.value,page:1})
		// console.log(user_articles_vedio);
		articleData.value = [articleData.value,user_articles_vedio.data]
	}
	...
	async function operCollecTions(status,index,articleId){
		if(status){//取消点赞
			...
			await requestApi('/collect-article',{article_id:articleId})
	...
	async function changeSwiper(event){
		...
		if(articleData.value.length >= 3){
			if(pageCount % 3 === 0){
				apiPage.value++
				const user_articles_vedio = await requestApi('/rec-the-video',{article_id:articleId.value,page:apiPage.value})
				articleData.value = [...articleData.value,...user_articles_vedio.data]
			}
		}
	}
	...
	function juMp(){
		wx.navigateTo({
			url:'/pages/search-article/search'
		})
	}
	// 返回上一页
	function fanHui(){
