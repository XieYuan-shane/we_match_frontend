...
<view class="companion-view" v-for="(item,index) in companionData" :key="index" @click="juMp(item._id)">
	...
	<view class="companion-time global-display global-a-items">
		...
		<text>地点：&nbsp; {{item.full_address}}</text>
	</view>
	<view class="companion-time global-display global-a-items">
		<image src="/static/icon/concern/category.png" mode="widthFix"></image>
...
<view class="sign-up other-up" @click="initPartner">发起结伴</view>
</template>
...
<script setup>
	...
	watch([keyWord,cityWord],async(newVal,oldVal)=>{
		page.value = 1
		const companionRes = await requestApi('/companion-query',
		{category:cityWord.value,keyword:keyWord.value,page:1}
		);
		exist.value = companionRes.data.length > 0 ? false : true
		companionData.value = companionRes.data
	})
	...
	onReachBottom(async()=>{
		page.value++
		const companionRes = await requestApi('/companion-query',
		{category:cityWord.value,keyword:keyWord.value,page:page.value}
		);
