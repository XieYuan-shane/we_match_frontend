<template>
	...
	<view class="init-textarea">
		<textarea v-model="message" auto-focu placeholder="说出你的结伴计划吧,比如时间安排,集合地点等" />
	</view>
	...
	<view class="personal-view global-display global-a-items">
		<text>目的地</text>
		<input type="text" v-model="poiname"  placeholder="地点">		
	</view>
	<view class="personal-view global-display global-a-items">
		<text>结伴时间</text>
...
<script setup>
	...
	// 选择结伴时间
	const startTime = ref('')
	const currentMonth = moment().format('YYYY-MM-DD'); // 当前日期时间
	...
	// 暂存展示的图片
	const fileList = ref([]);
	const categories = ['学习', '娱乐', '活动', '交易', '其他'];
	// 选择种类
	const category = ref("")
	// 监听必填值是否填写完毕
	...
	onLoad(async(event)=>{
	    if(event.articleId){
			...
			const article_data = await requestApi('/article-data',{article_id:articleId})
			const articleData = article_data.data;
			console.log(articleData)
			poiname.value = articleData.address;
			message.value = articleData.content;
			category.value = articleData.category;
			fileList.value = articleData.image.map(url => ({ tempFilePath: url, isNew: false }));
	    }
	})
	// 上传图片
		...
		wx.chooseMedia({
		  ...
		  success:async(res)=> {
			res.tempFiles.forEach(item=>{
				fileList.value.push({...item, isNew: true});
			})
		  }
		})
	}
	...
	watchEffect(()=>{
		const hasData = message.value && fileList.value.length > 0  && poiname.value && startTime.value && numberOfPeople.value > 0;
	...
	async function subMit(){
	  wx.showLoading({title: '提交中',mask:true});
	    await requestApi('/check-login'); // 检查是否登录
		
	    let finalImageUrls = fileList.value.map(file => {
		   if (file.isNew) {
			   return null;
		   } else {
	   ...
	   const newFiles = fileList.value.filter(file => file.isNew);
	   if (newFiles.length > 0) {
		   const newFilesUrls = await upLoadCos(newFiles);
		   let newUrlIndex = 0;
		   finalImageUrls = finalImageUrls.map(url => {
			   if (url === null) {
				   const newUrl = newFilesUrls[newUrlIndex];
				   newUrlIndex++;
				   return newUrl;
			   }
			   return url;
