<template>
	<view class="mine-top fadeIn">
	  ...
	  <view class="mine-user">
			...
			<view class="age-view global-display global-a-items">
				...
				<text v-if="userInfor.college != ''">{{userInfor.college}}</text>
			</view>
			<!-- uid -->
	...
	<Mywork :userArticles="userArticles" :tip="tip"/>
...
<script setup>
	...
	try{
		...
		// 查询我发布的文章
		const myArticle = await requestApi('/myArticles',{page:1});
		userArticles.value = myArticle.data
	...
	function getUserInfor(){
		const {avatarUrl,backdrop,nickname,uid,age,college} = uni.getStorageSync('userInfor')
		userInfor.avatarUrl = avatarUrl;
		userInfor.backdrop = backdrop;
		userInfor.nickname = nickname;
		userInfor.uid = uid;
		userInfor.age = age;
		userInfor.college = college;
	};
	const tabNav = ref(['作品','收藏','喜欢'])
	// 存储我的作品收藏点赞接口
	const apiRe = ref(['/myArticles','/myCollEction','/myLikeArticle'])
	//存储选项卡选中哪个
	const tabIndex = ref(0)
	// 存储提示
	const tipData = ref(['你还没有发布作品','快去收藏一些活动吧','你还没有喜欢的活动/文章'])
	...
	async function handleTabTap(e) {
		...
		page.value = 1//再次查询时要把页数置为1
		if(index === 0){//作品
			const myArticle = await requestApi(apiRe.value[0],{page:1});
			userArticles.value = myArticle.data
			
		}else if(index === 1){//收藏
			const myArticle = await requestApi(apiRe.value[1],{page:1});
			userArticles.value = myArticle.data
			
		}else{//喜欢
			const myArticle = await requestApi(apiRe.value[2],{page:1});
			userArticles.value = myArticle.data
		}
	}
	// 上拉加载
	...
	onReachBottom(async()=>{
		...
		const user_articles = await requestApi(API,{page:page.value})
